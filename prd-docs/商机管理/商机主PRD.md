...
> **阶段口径**：商机业务主链路包含 6 个节点（`INITIAL_CONTACT`、`NEEDS_CONFIRM`、`PROPOSAL`、`NEGOTIATION`、`CONTRACT`、`WON`），另有 1 个输单终态 `LOST`，合计 7 个状态。本文及相关 Demo 不将“六阶段”和“七状态”混用。
...
- 通用列表规范默认按创建时间倒序；商机模块因销售管道的业务需要，明确例外为按“阶段变更时间”倒序。
- 同一阶段变更时间下，依次按创建时间倒序、商机编号倒序，保证排序稳定。
...
- 商机保存 `crmCustomerId`、`erpCustomerId`、`customerSnapshotVersion` 和 `customerSnapshotSyncTime`；CRM 客户名称仅用于展示，不作为跨系统关联键。
- `crmCustomerId` 关联 CRM 客户快照，用于 CRM 内展示、权限校验和历史上下文；`erpCustomerId` 用于 ERP 订单下推。
- 客户快照用于展示和权限，正式客户档案及其主数据仍由 ERP 负责；CRM 不在商机流程内创建、修改或以名称匹配 ERP 客户。
- 下推前校验客户快照版本和同步有效性；失效时阻断下推并要求重新选择或等待 ERP 同步。

#### 3.4 ERP 销售订单草稿下推契约

商机进入 `WON` 后，CRM 以稳定的 `ERP_ORDER_IDEMPOTENCY_KEY` 调用 ERP Mock 创建销售订单草稿。以下契约是 CRM 与 ERP 的边界，ERP 正式订单仍由 ERP 负责。

请求字段：

| 字段 | 必填 | 来源/规则 |
|------|:----:|----------|
| `crmOpportunityId` | 是 | CRM 商机唯一标识 |
| `erpCustomerId` | 是 | ERP 客户档案标识；不得使用客户名称替代 |
| `customerSnapshotVersion` | 是 | CRM 保存的客户快照版本，用于校验引用事实 |
| `items` | 是 | 商品明细数组；每行至少包含 `erpProductId`、`productCode`、`quantity`、`unitPriceSnapshot`、`priceVersion`，必要时带 `productNameSnapshot` |
| `crmContractId` | 是 | 已签署合同标识；对外契约统一使用 `crmContractId`，模块内部简称 `contractId` 仅限本地展示/变量 |
| `contractSignedAmount` | 是 | 取关联合同的签署金额事实，必须同时传 `currency`、`taxIncluded`、`taxRate`；这是 ERP 请求金额输入 |
| `currency` | 是 | 取关联合同签署金额币种；一期固定 `CNY` |
| `taxIncluded` | 是 | 取关联合同签署金额含税标识 |
| `taxRate` | 是 | 取关联合同签署税率 |
| `expectedAmount` | 否 | CRM 预计金额，仅用于预测和复盘，不进入 ERP 请求、不作为正式订单金额 |
| `businessDate` | 是 | 商机业务发生日期/赢单业务日期 |
| `idempotencyKey` | 是 | 与 CRM 的 `ERP_ORDER_IDEMPOTENCY_KEY` 相同；同一商机重试复用 |

返回字段：

| 字段 | 必填 | 说明 |
|------|:----:|------|
| `erpOrderDraftId` | 成功时是 | ERP 销售订单草稿编号；失败或处理中可为空 |
| `syncStatus` | 是 | `NOT_TRIGGERED/PROCESSING/SUCCESS/FAILED/RETRYING` |
| `errorCode` | 失败时是 | ERP 或网关返回的机器可读错误码 |
| `errorMessage` | 失败时是 | 面向 CRM 记录和展示的错误摘要 |

下推边界：

- 客户和商品只发送 ERP 标识及创建订单所需的必要快照；CRM 不自行创建客户或商品主数据，也不把客户名称当作关联键。
- ERP 下推金额来源固定为关联合同的 `contractSignedAmount`，并携带 `currency`、`taxIncluded`、`taxRate`；`expectedAmount` 仅为 CRM 预测字段，不得下推。
- 一期仅支持 `CNY`；非 CNY 合同在创建/提交签署时阻断并提示“一期仅支持人民币结算”，不静默换算。ERP 必须以自身客户、商品、价格、数量和金额校验结果为准，ERP 返回的正式订单金额才是订单金额 SSOT。
- CRM 保存返回的 `erpOrderDraftId`、`syncStatus`、错误信息、同步时间和幂等键，作为同步记录；ERP 正式订单详情、审核和履约不在 CRM 维护。
- 超时或失败只改变 ERP 订单同步结果字段，不回退已确认的 `WON`。
...
| `CONTRACT` | 合同签署完成 | 回调合法；合同与商机绑定一致 | `WON` | 否 | 写入赢单时间；以关联合同签署金额（含 `currency`/`taxIncluded`/`taxRate`）下推 ERP 订单草稿；更新业绩 | 回调失败保持原状态；记录错误；Toast `签署结果同步失败` |
| `CONTRACT` | 合同作废 | 合同允许作废；原因已填写 | `NEGOTIATION` | 是 | 解除合同绑定；商机恢复可推进 | 合同作废事实保留；商机回退失败进入“联动待补偿”并提供主管/管理员重试；Toast `合同已作废，商机回退待补偿` |
...
| 动作 | INITIAL_CONTACT | NEEDS_CONFIRM | PROPOSAL | NEGOTIATION | CONTRACT | WON | LOST |
|------|:------:|:-----:|:--------:|:-----------:|:--------:|:---:|:----:|
| 查看 | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| 编辑 | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| 添加跟进 | ✅ | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ |
...
`CONTRACT_LINK_STATUS` 是合同与商机之间的联动记录，独立于商机主状态（`INITIAL_CONTACT` 等 7 个状态），不得作为表单字段直接编辑，也不得用它替代主状态机。

| 子状态 | 进入条件 | 允许动作/后置处理 | 与商机主状态关系 |
|--------|----------|------------------|------------------|
| `DRAFT`（合同草稿） | `NEGOTIATION` 点击“发起合同”且合同创建成功 | 可查看合同；合同创建失败不产生该状态 | 商机进入 `CONTRACT` 后记录 |
| `PENDING_SIGN`（待签署） | 合同草稿提交签署 | 等待合法签署事件；不允许手工赢单 | 商机保持 `CONTRACT` |
| `SIGNED_PENDING_SYNC`（签署完成待同步） | 收到签署完成事件，验签通过且合同绑定一致，正在写入 CRM/后续同步 | 事件按 `eventId` 去重；等待联动完成 | 联动事务完成前保持原主状态；完成后由合法事件将 `CONTRACT` 置为 `WON` |
| `SYNC_FAILED`（同步失败） | 合法签署事实已保存，但合同与商机/后续 ERP 联动失败 | 记录错误并提供“人工重试” | 不因失败直接改写主状态；若已进入 `WON` 则保持 `WON` |
| `VOIDED`（作废） | 合同作废动作成功 | 保留历史合同与签署/作废记录 | 合法作废事件将 `CONTRACT` 回退 `NEGOTIATION`；不修改已确认的 `WON` |
| `MANUAL_RETRY`（人工重试） | 主管或管理员点击“人工重试” | 复用原合同/事件标识，成功则进入后续联动，失败回到 `SYNC_FAILED` | 只代表联动动作，不代表商机阶段 |

补充约束：

- 合同创建失败保持 `NEGOTIATION`；重试成功后才进入 `CONTRACT`。
- 签署事件必须包含 `crmContractId`、`crmOpportunityId`、`eventId` 和签署结果；重复事件只允许执行一次赢单联动。对外契约使用 `crmContractId + crmOpportunityId`；商机模块内部可将前者简称 `contractId`，但跨模块事件/请求不得使用简称。
- **字段映射说明**：对外签署事件契约统一使用 `crmContractId + crmOpportunityId`；商机模块内部可将 `crmContractId` 简称为 `contractId`，合同模块内部可将 `crmOpportunityId` 简称为 `opportunityId`，但跨模块事件、请求、日志和验收数据必须使用对外统一名称。
- 验签失败保持 `CONTRACT`，不写入签署完成事实，不触发 `WON`。
- 合同作废必须通过合同模块合法动作或事件触发，商机回退 `NEGOTIATION` 并保留历史合同，不通过商机表单修改状态。
...
| 场景 | 处理方式 |
|------|----------|
| 两人同时推进同一商机 | **推荐实现（非业务约束）**：以版本号乐观锁控制，后提交者失败并刷新最新阶段；具体实现可按技术架构选型 |
...
| 重复 ERP 订单下推 | 业务要求是以商机标识对应的 `ERP_ORDER_IDEMPOTENCY_KEY` 去重，禁止生成第二份草稿；**推荐实现（非业务约束）**：服务端可使用 Redis 短期请求锁/幂等缓存并以持久化记录兜底 |
...
| 2026-08-13 | V2.1 | 补齐客户/商品双标识、ERP 下推契约、合同联动子状态、排序例外与推荐实现说明 |
