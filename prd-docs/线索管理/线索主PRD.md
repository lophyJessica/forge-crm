...
- **超时回收机制**：对已分配且超过 48 小时仍无跟进记录的线索，在列表数据更新时检查并自动回收，清除归属关系；当前原型不覆盖跟进中超时回收，也不实现 10 分钟定时扫描。
...
flowchart TD
    ...
    N -->|填写放弃原因并通过非空校验| O["状态变更为 ABANDONED (进入公海)"]
    O -->|原负责人可立即撤销放弃| J
...
| S05 | 销售主动放弃线索 | **支线** | **分支路径描述**：销售在取得联系后判定线索无效，在详情页点击“放弃”按钮。系统弹出弹窗，要求填写 `放弃原因`，当前原型仅做非空校验。保存后将状态变更为 `ABANDONED`，写入放弃原因、更新页面时间字段并追加放弃跟进记录；当前原型不清除 `归属人` 与 `分配时间`，原负责人可立即从公海撤销放弃并恢复为 `ASSIGNED`。 |
| S06 | 公海线索认领 | **支线** | **分支路径描述**：销售人员在“公海池”列表中浏览可见的 `ABANDONED` 状态线索。原负责人可立即撤销放弃；其他销售在该线索已作废超过 7 天后可见并认领。认领成功后，系统将 `归属人` 修改为认领销售，`线索状态` 变更为 `ASSIGNED`，写入当前系统时间至 `分配时间`。 |
...
stateDiagram-v2
    ...
    ABANDONED --> ASSIGNED : 原负责人可立即撤销放弃；其他销售已作废超过7天后可认领
```
...
| `ASSIGNED` | 主动放弃 | 操作人为当前 `归属人`，`放弃原因` 必填且通过非空校验 | `ABANDONED` | 是 | 写入 `放弃原因`，更新页面时间字段并追加放弃跟进记录，移入公海；当前原型不清空 `归属人` 与 `分配时间`，原负责人可立即撤销放弃。 | 放弃原因为空时阻断；若此期间状态已变更，刷新列表后重试。 |
...
| `FOLLOWING` | 主动放弃 | 操作人为当前 `归属人`，`放弃原因` 必填且通过非空校验 | `ABANDONED` | 是 | 写入 `放弃原因`，更新页面时间字段并追加放弃跟进记录，进入公海；当前原型不清空 `归属人` 与 `分配时间`，原负责人可立即撤销放弃。 | 放弃原因为空时阻断；若状态已变更，刷新列表后重试。 |
| `ABANDONED` | 公海认领/撤销放弃 | 原负责人可立即操作；其他销售在已作废超过 7 天后可见并认领 | `ASSIGNED` | 是 | `归属人` 变更为当前认领销售，写入当前系统时间至 `分配时间`，恢复跟进。 | 线索尚未对当前销售可见时不展示认领入口。 |
...
- **R04 [已分配线索超时回收规则]**：列表数据发生更新时检查状态为 `ASSIGNED` 且存在 `分配时间` 的线索。若当前时间距 `分配时间` 已超过 48 小时，且该线索没有任何跟进记录，则清空 `归属人` 与 `分配时间`，将状态回退至 `PENDING_ASSIGN`。当前原型不覆盖 `FOLLOWING` 状态超时回收，也不包含 10 分钟定时扫描。
...
- **R07 [放弃原因必填约束]**：当销售在详情页点击“放弃”使线索流入公海时，系统必须弹出输入框，要求 `放弃原因` 必填；当前原型仅执行非空校验，不要求至少 15 字。
...
- **R10 [公海认领与撤销放弃规则]**：当销售主动放弃某条线索导致其状态流转为 `ABANDONED` 时，原负责人可立即从公海撤销放弃并恢复为 `ASSIGNED`。其他销售在该线索已作废超过 7 天后可见并认领；该 7 天是其他销售的公海可见条件，不作为原负责人的撤销保护期。
...
| V05 | 公海撤销放弃与认领 | 销售 A 主动放弃名下一条线索并填写非空放弃原因，线索状态变更为 `ABANDONED` 进入公海。销售 A 立即尝试撤销放弃，销售 B 在该线索已作废超过 7 天后尝试认领。 | 1. 销售 A 可立即撤销放弃，线索状态转为 `ASSIGNED`，归属人仍为销售 A，`分配时间` 更新为当前时间。<br>2. 销售 B 在超过 7 天后可见并认领，线索状态转为 `ASSIGNED`，`分配时间` 写入当前认领时间。<br>3. 放弃原因为空时，放弃动作被阻断。 |
