...
| S05 | 销售主动放弃线索 | **支线** | **分支路径描述**：销售在取得联系后判定线索无效，在详情页点击“放弃”按钮。系统弹出弹窗，要求填写至少 15 字的 `放弃原因`。保存后将状态变更为 `ABANDONED`，写入放弃原因、更新页面时间字段并追加放弃跟进记录；保留 `归属人`、清空 `分配时间`，原负责人可立即从公海撤销放弃并恢复为 `ASSIGNED`。 |
...
| `ABANDONED` | 已作废/已放弃（线索被判定无效、被销售放弃，流回公海；原负责人可立即撤销放弃） | **否 (可恢复为已分配)** |
...
stateDiagram-v2
    ...
    ABANDONED --> ASSIGNED : 原负责人可立即撤销；防撞墙仅约束其他销售，其他销售超过7天后可认领
```
...
| `ASSIGNED` | 主动放弃 | 操作人为当前 `归属人`，`放弃原因` 至少 15 字且通过校验 | `ABANDONED` | 是 | 写入 `放弃原因`，更新页面时间字段并追加放弃跟进记录，移入公海；保留 `归属人`，清空 `分配时间`，原负责人可立即撤销放弃。 | 放弃原因少于 15 字时阻断并提示“放弃原因至少15字”；若此期间状态已变更，刷新列表后重试。 |
...
| `FOLLOWING` | 主动放弃 | 操作人为当前 `归属人`，`放弃原因` 至少 15 字且通过校验 | `ABANDONED` | 是 | 写入 `放弃原因`，更新页面时间字段并追加放弃跟进记录，进入公海；保留 `归属人`，清空 `分配时间`，原负责人可立即撤销放弃。 | 放弃原因少于 15 字时阻断并提示“放弃原因至少15字”；若状态已变更，刷新列表后重试。 |
...
- **R06 [终态只读保护规则]**：一旦线索进入 `CONVERTED`（已转客户）状态，除了系统管理员角色外，普通销售与主管均无权再对其基本信息进行任何编辑修改。`ABANDONED` 不是不可恢复终态，原负责人可从公海立即撤销放弃并恢复为 `ASSIGNED`。
- **R07 [放弃原因长度约束]**：当销售在详情页点击“放弃”使线索流入公海时，系统必须弹出输入框，要求 `放弃原因` 至少 15 字；不足 15 字时阻断放弃动作并提示“放弃原因至少15字”。
...
#### 8.1 数据可见范围
- **销售角色**：采用行级数据权限隔离。在线索列表中可查看自己作为 `归属人` 的线索（包括 `DRAFT`、`ASSIGNED`、`FOLLOWING`）。在“线索公海”列表中，可查看 `PENDING_ASSIGN`；自己放弃的 `ABANDONED` 线索可立即查看并撤销放弃，其他销售仅能查看已作废超过 7 天的公海线索。
  *数据库查询限制*：`WHERE owner_id = :current_user_id OR status = 'PENDING_ASSIGN' OR (status = 'ABANDONED' AND (owner_id = :current_user_id OR current_time - abandon_time > 7 days))`。
...
- **管理员手动分配与列表回收检查并发**：当管理员在后台手工分配某条线索，而列表数据更新同时触发该线索的超时回收检查时，系统必须对分配和回收接口加乐观锁保护。如果修改时检测到线索版本号已变，操作回滚，并给管理员提示“操作失败，线索状态已变更，请刷新列表重试”。
