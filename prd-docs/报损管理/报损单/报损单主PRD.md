...
| 报损期间库存冻结 | 系统 | 提交审核或自动生成待审核后冻结报损数量，防止继续销售/出库；具体冻结时点见不确定性 |
| 一道审核 | PC | 主管或财务审核报损，作为资产核销的合法环节 |
| 确认核销 | PC/系统 | 审核通过并确认核销后，现存 `-N`、冻结 `-N`，生成来源为 BL 的 FL |
...
| 4 | 调拨损耗自动生成 | TR 调出 80 件，调入实收 76 件 | 系统生成 BL，原因固定调拨损耗，待审核后核销差异 4 件 |
...
| 待审核 | 等待主管/财务审核 | 审核通过并确认核销、审核驳回 | 已冻结报损数量，现存不变 |
| 已审核 | 已完成资产核销 | 查看详情、查看 FL | 现存 `-N`，冻结 `-N`，生成来源 BL 的 FL |
...
| 分类 | 核心字段 |
|:--|:--|
| 报损头 | 报损单号、来源、关联单号、报损类型、报损原因、申请人、审核人、状态、提交时间、审核时间、核销时间 |
...
| R4 | 报损冻结 | 提交审核或自动生成待审核后冻结报损数量，防止继续出库 |
| R5 | 审核必经 | 报损涉及资产核销，必须经一道主管/财务审核 |
| R6 | 核销时点 | 仅“审核通过并确认核销”后，现存 `-N` 并生成来源 BL 的 FL |
| R7 | 驳回 | 驳回不扣减现存，并解除本次报损冻结 |
| R8 | 三口径 | 可用 = 现存 - 占用 - 冻结；草稿/待审核不扣现存，核销后现存减少 |
...
| AC7 | 审核核销 | 审核通过并确认核销后现存 -N，生成来源为 BL 的 FL |
...
| 项 | 当前处理 | 待确认点 |
|:--|:--|:--|
| 报损冻结时点 | 本套件按“提交审核/自动生成待审核后冻结，草稿不冻结”处理 | context 只说明报损触发冻结，未明确具体时点 |
| 冻结/解冻是否单独生成 FL | 本套件强制核销生成来源 BL 的 `STOCK_LOSS` FL；冻结/解冻可按库存流水模块生成 `FREEZE`/`UNFREEZE` | 是否所有报损冻结都要在列表展示为独立 FL，需库存流水口径复核 |
| 调拨差异待核销口径 | 本套件按 BL 审核完成作为资产核销合法时点；若 TR 已先消减在途，BL 不得重复扣减同一库存 | TR 与 BL 的库存台账落地时点需研发/产品统一 |
