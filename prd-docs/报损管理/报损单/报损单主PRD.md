...
| 驳回 | PC | 审核不通过进入已驳回；手动报损解除本次报损冻结，调拨损耗差异待核销持续挂账，库存现存不变 |
...
| 5 | 审核驳回 | 审核发现数量或原因不成立 | BL 进入已驳回；手动报损解除本次冻结，调拨损耗差异待核销保持挂账、不回补现存 |
...
| 已核销 | 已完成资产核销 | 查看详情、查看 FL | 手动报损现存 `-N`；调拨损耗差异待核销 `-N`；生成来源 BL 的 FL |
| 已驳回 | 审核不通过 | 查看详情 | 手动报损解除本次报损冻结；调拨损耗差异待核销 D 件保持挂账、不回补现存，允许对该差异重新生成 BL 或转人工核实 |
...
| R7 | 驳回 | 驳回不扣减现存；手动报损解除本次冻结，调拨损耗差异待核销在重新核销前持续挂账，可重新生成 BL 或转人工核实 |
