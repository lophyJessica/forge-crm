...
报损单（BL）用于处理仓库内不可继续销售、不可继续出库或调拨到货短少形成的库存报损核销。普通仓内短少（非调拨）归属 `OTHER` 类型；与盘点盘亏的分工：盘点发现的短少走盘点单 CK → 盘亏 `STOCK_LOSS`，不走报损单 BL。报损涉及企业资产减少，必须经过合法审核后才能核销库存；草稿、待审核阶段不得直接扣减现存。
...
| 驳回 | PC | 审核不通过时填写驳回原因，状态进入已驳回；不扣现存、不生成 FL，允许回到草稿修改 |
...
| 5 | 审核驳回 | 审核发现数量或原因不成立 | BL 进入已驳回并记录驳回原因；不扣现存、不生成 FL，申请人可回到草稿修改后重新提交 |
...
| 状态 | 含义 | 可执行动作 | 库存影响 |
|:--|:--|:--|:--|
| 草稿（DRAFT） | 手动创建后尚未提交审核，或驳回后回到可编辑状态 | 编辑、保存、提交审核 | 不冻结、不扣减现存 |
| 待审核（PENDING_REVIEW） | 等待主管/财务审核 | 审核通过并确认核销、审核驳回 | 手动报损已冻结报损数量；调拨损耗已锁定差异待核销；现存不变 |
| 已核销（CONFIRMED） | 已完成资产核销 | 查看详情、查看 FL | 手动报损现存 `-N`；调拨损耗差异待核销 `-N`；生成来源 BL 的 FL |
| 已驳回（REJECTED） | 审核不通过，已记录驳回原因 | 查看驳回原因、修改草稿、重新提交 | 不扣现存、不生成 FL |
...
| R7 | 驳回 | 驳回必须填写原因；状态进入已驳回，不扣减现存、不生成 FL，允许回到草稿修改后重新提交 |
