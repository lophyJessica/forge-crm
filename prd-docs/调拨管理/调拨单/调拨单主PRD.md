...
调拨单增加主管审核流。状态变更必须由动作按钮触发，不允许直接编辑状态字段。草稿只能「保存草稿」或「提交审核」，审核通过后才允许确认调出。
...
| 状态 | 含义 | 可执行动作 | 进入条件 |
|:--|:--|:--|:--|
| 草稿 | TR 已创建，尚未提交审核或驳回后待修改 | 保存草稿、提交审核、查看详情 | WMS 手动创建、ERP 下发或审核驳回 |
| 待审核 | TR 已提交，等待主管审核 | 审核通过、审核驳回、查看详情 | 点击提交审核 |
| 已审核 | 主管已确认调拨合理，尚未调出 | 确认调出、查看详情 | 主管审核通过 |
| 已驳回 | 主管审核不通过，已记录驳回原因 | 修改草稿、重新提交审核 | 主管点击驳回；不调整库存、不生成 FL |
...
| R3 | 审核必经 | 调拨单必须先提交主管审核；审核通过后才允许确认调出 |
| R4 | 审核驳回 | 驳回必须填写原因；状态进入已驳回，不调整库存、不生成 FL，允许修改后重新提交 |
| R5 | 调出确认 | 调出库确认后，调出仓现存-N，调拨在途+N |
| R6 | 在途不可销售 | 在途库存处于调出确认到调入确认之间，不计入可用，不可销售 |
| R7 | 调入确认 | 实收等于调出时，调入仓现存+N，调拨在途-N |
| R8 | 差异入库 | 实收小于调出时，调入仓现存+R；在途-R；差异 D=N-R 转「差异待核销」（在途-D、差异待核销+D） |
| R9 | 差异报损 | 到货差异自动生成 BL（待审核），单号按 `BL{YYYYMMDD}-{4位序号}`，原因=`调拨损耗`；TR 完成≠差异已核销 |
| R10 | 字段口径 | 字段唯一事实来源为 `调拨单字段清单.md` |
...
| AC4 | 状态流转 | 草稿 → 待审核 → 已审核 → 已调出（在途） → 已调入（完成）；待审核可驳回到已驳回并回草稿修改，全部由动作按钮触发 |
| AC5 | 审核控制 | 未审核通过的 TR 不允许确认调出；驳回不调整库存、不生成 FL |
| AC6 | 调出过账 | 确认调出后调出仓现存-N，调拨在途+N |
| AC7 | 在途限制 | 在途库存不可销售，不计入可用 |
| AC8 | 调入过账 | 正常调入后调入仓现存+N，调拨在途-N |
| AC9 | 差异处理 | 实收小于调出时按实收 R 入库，差异 D 转「差异待核销」，并生成原因=调拨损耗的 BL（待审核）；TR 完成≠差异已核销 |
| AC10 | Demo 数据 | 用例数据使用 2026 示例，清楚展示两仓现存/在途变化 |
