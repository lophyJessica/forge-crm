...
| 待审核 | 全部明细已实盘，差异报告已生成 | 主管重盘、审核通过并确认调整 | 全部明细完成且系统完成差异比对 |
...
| AC2 | 状态流转 | 仅允许草稿 → 盘点中 → 待审核 → 已调整；差异率 >5% 时待审核 → 盘点中 |
...
| AC9 | 库存调整 | 确认调整后，盘盈现存增加、盘亏现存减少，并生成来源为 CK 的 FL |
| AC10 | 单据边界 | 页面和规则不出现独立差异调整单入口 |
