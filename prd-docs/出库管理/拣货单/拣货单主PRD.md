...
| # | 场景 | 示例 | 系统处理 |
|:--:|:--|:--|:--|
| 1 | 正常拣货 | 应拣 SKU-A100 10 件，货位和商品扫描正确，实拣 10 件 | 明细完成，累计进度更新 |
| 2 | 扫错货位 | 当前应扫 A-01-02，误扫 A-01-03 | PDA 报错并语音/震动提示，不允许确认 |
| 3 | 扫错商品 | 当前应拣 SKU-A100，误扫 SKU-B200 | PDA 报错阻断，不允许录入数量 |
...
| 拣货中 | 拣货员正在 PDA 扫码执行 | 扫货位、扫商品、确认数量、登记缺货 | PDA 领取任务或首条明细开始 |
...
| R8 | 库存过账 | 拣货确认不扣减现存库存；包装完成触发库存扣减和 FL |
...
| AC8 | 库存口径 | 拣货完成不生成 FL、不扣减现存；包装完成才扣减库存 |
| AC9 | 页面规范 | PDA 大字体、大按钮、扫码优先、关键操作语音+震动反馈 |

## 9. 不确定性

- `context/05` 明确“包装完成=库存扣减触发点”，`context/06` 将出库动作概括为“PDA拣货→交运确认”。本文按更明确的 `context/05` 处理：拣货确认只记录实拣结果，不扣减现存、不生成 FL；包装完成才扣减库存并生成 FL。
