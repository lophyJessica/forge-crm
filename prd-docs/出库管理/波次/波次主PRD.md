...
| 分配拣货员 | PC | 对草稿波次分配拣货员，准备下推拣货任务 |
| 下推拣货 | 系统/PC | 波次进入拣货中后生成或关联 PICK |
| 波次关闭 | PC | 草稿或作业中无法继续处理的波次可作废，需二次确认 |
...
| 4 | 分配拣货员 | 草稿波次 WAVE20260705-0001 | 主管分配拣货员，进入拣货中 |
| 5 | 下游完成 | 关联 PICK/CHECK/PKG/DSH 全部完成 | 波次状态变更为已交运 |
| 6 | 异常关闭 | 波次尚未开始，订单取消或线路异常 | 二次确认后作废波次，释放组织关系；库存释放以订单取消规则为准 |

## 5. 状态机摘要

波次状态表达组织单元自身的推进情况，完全对齐出库流程主状态。
...
| 状态 | 含义 | 可执行动作 | 说明 |
|:--|:--|:--|:--|
| 草稿 | 波次已生成，尚未开始拣货 | 分配人员、下推拣货、作废 | 对应主流程草稿/待作业状态 |
| 拣货中 | 拣货任务已下推并正在执行 | 查看进度、异常关闭 | 拣货执行细节在 PICK 中处理 |
| 已拣货 | 关联的拣货单已全部拣完 | 查看进度、异常关闭 | 待下游复核 |
| 已复核 | 复核完成且包装称重贴面单完毕 | 确认交运、异常关闭 | 待交运出库 |
| 已交运 | 包裹已交接承运商，出库任务完结 | 查看详情 | 终态，不可更改 |
| 已作废 | 波次被取消或异常关闭 | 查看详情 | 终态，不物理删除 |
...
| R7 | 库存扣减口径 | 包装完成为库存扣减触发点（现存-N、释放占用-N，可用不变；并生成 FL），波次生成不扣减库存 |
...
| AC5 | 状态流转 | 草稿、拣货中、已拣货、已复核、已交运、已作废只能通过动作按钮变化 |
| AC6 | 库存口径 | 波次生成不扣减库存；包装完成才扣减现存并释放占用（现存-N、占用-N，可用不变；并生成 FL） |
| AC7 | 页面规范 | PC 表格默认 20 条/页，作废等危险操作需二次确认，按钮不可用时隐藏 |
