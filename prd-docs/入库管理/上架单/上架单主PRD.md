...
| 功能 | 端 | 说明 |
|:--|:--|:--|
| 质检合格生成上架单 | 系统 | 收货单 RCV 完成质检登记且合格数量 > 0 时系统自动生成 PUT，全不合格不可上架 |
| 推荐货位 | 系统 | 根据仓库、商品和货位空闲情况推荐货位，空闲货位优先 |
| PDA 领取上架任务 | PDA | 仓管查看待上架任务并开始作业 |
| 扫描货位 | PDA | 扫描实际货位条码，带入实际上架货位 |
| 确认上架数量 | PDA | 输入本次上架数量，支持一单多货位分批上架 |
| PDA分批即时过账 | 系统 | PDA 每次确认上架时，即时按实际上架货位将该批数量的库存从冻结转为可用并即时生成对应的 FL |
| PC 列表/详情查看 | PC | 查看上架任务、进度、货位和库存流水关联 |

### 2.2 Out Scope

- 不提供无来源手工新增上架单；PUT 必须来自收货单 RCV 的质检合格品。
- 不增加审核流，不出现待审核、已审核、反审核等状态。
- 质检结果直接登记在收货单内并快照继承到上架单中，上架单自身只读取合格品数量，不负责登记或更改质检信息。
...
| 业务定位 | 记录合格品最终放入哪个货位，是触发库存转为现存可用的凭证 |
| 生成方式 | 收货单 RCV 完成质检登记且合格数量 > 0 后系统自动生成 |

> 口径说明：上架单完全由收货单质检登记的合格品触发生成。

## 4. 业务场景

| # | 场景 | 示例 | 系统处理 |
|:--:|:--|:--|:--|
| 1 | 正常上架 | 合格 100 件，推荐货位 A-01-02，一次上架 100 件 | 提交确认，即时扣减冻结库存 +100 可用，即时生成 FL，PUT 状态变为已完成 |
| 2 | 一单多货位 | 合格 100 件，A-01-02 上 60 件，A-01-03 上 40 件 | 第一次确认后 PUT 为上架中，即时将 60 件从冻结转可用并生成 FL1；第二次确认 40 件，即时将 40 件转可用并生成 FL2，累计 100 达标，PUT 变为已完成 |
...
| 6 | 质检全不合格 | RCV 质检合格数量 = 0 | 收货单转为异常状态，不生成 PUT，不允许上架 |
...
| 状态 | 含义 | 可执行动作 | 进入条件 |
|:--|:--|:--|:--|
| 待上架 | 已生成上架任务，尚未确认上架 | PDA 上架确认 | 收货单质检登记后合格数 > 0 |
...
| # | 规则 | 摘要 |
|:--:|:--|:--|
| R1 | 质检前置 | 仅收货单质检合格数量>0 可生成 PUT；质检未登记或全不合格不可上架 |
...
| R7 | 分批即时过账 | PDA 每次确认上架时即时将该批数量库存从冻结转可用，并即时生成对应的库存流水 FL |
| R8 | 快照追溯 | 商品、合格数量、来源 RCV 在 PUT 中快照存储 |

## 7. 字段清单入口

字段 the 唯一事实来源见 `上架单字段清单.md`。本主 PRD 不重复维护完整字段定义，只保留核心字段摘要：
...
| # | 验收项 | 验收标准 |
|:--:|:--|:--|
| AC1 | 前置控制 | 收货单未完成质检登记或合格数量=0 时，不生成 PUT 且不可上架 |
...
| AC7 | 过账时点 | PDA 每次确认上架时即时过账（冻结转可用，并生成对应 FL），而非整单完成才过账 |
