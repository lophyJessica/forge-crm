...
| 截图 | 页面说明 |
|------|---------|
| ![新建线索页](./images/lead-form.png) | **功能点**：新增模式空白表单，可保存草稿或提交；编辑模式加载已有线索，仅草稿态可编辑；三个动作（返回列表/保存草稿/提交）。<br>**业务关系**：新增/编辑是线索对象录入维护入口；提交后进入AI评分分流（见5.2）；转客户后客户档案权威是ERP。<br>**交互规则**：保存草稿暂存未完成且格式错阻断；提交先校验完整性再去重，通过后评分分流失败保留表单；手机/邮箱失焦各自校验标红；先选省再载市；防重复提交按钮loading；编辑非草稿只读。<br>**业务规则**：手机号邮箱至少填一个；去重命中任一历史线索阻断；公司/来源正式提交必填；首字段错误自动获得焦点。 |
...
| 截图 | 页面说明 |
|------|---------|
| ![线索详情页](./images/lead-detail-real.png) | **功能点**：展示完整资料（状态区/AI评分卡/基本信息/跟进时间线）；按状态显示底部操作栏；AI评分权重可展开；含添加跟进/放弃/转客户/草稿作废弹窗。<br>**业务关系**：详情是线索生命周期状态与动作执行点；转客户联动ERP建档；跟进/放弃/作废产生的痕迹进时间线。<br>**交互规则**：只通过动作按钮触发状态流转；按状态显示操作栏（草稿→编辑/提交/作废草稿；待分配→认领；已分配→跟进/放弃；跟进中→跟进/转客户/放弃；已转客户→查看ERP客户；已作废→按类型恢复/认领）；跟进弹窗必填、取消清空；放弃/转客户二次确认；防重复提交。<br>**业务规则**：详情展示CRM线索+客户快照不替代ERP主数据；转客户强一致（仅ERP成功才改已转客户，失败保持跟进中不写转化时间幂等）；已转客户只读终态；竞争失败提示刷新重试。 |
...
| 截图 | 弹窗说明 |
|------|---------|
| ![添加跟进记录弹窗](./images/modal-add-follow.png) | **添加跟进记录**：跟进方式（必填，电话/拜访/邮件）、跟进内容（必填，多行）、下次跟进计划（选填）；取消 / 提交记录；关闭X。<br>**交互**：内容为空时提交不可用；取消清空未提交内容；提交成功后刷新时间线并更新最近跟进时间。<br>**业务规则**：已分配线索首条有效跟进后状态变跟进中；跟进中追加保持跟进中。 |
| ![放弃线索确认弹窗](./images/modal-abandon.png) | **确认放弃线索**：黄色警告标识，说明放弃后果（进公海，原负责人7天不可认领、其他销售可立即认领）；请填≥15字原因，含"已输入 0/15 字"字数统计。<br>**交互**：原因为空或不足15字时确认按钮禁用；确认后写原因、更新状态、追加放弃跟进记录。<br>**业务规则**：放弃返回已作废，保留归属人、清空分配时间；原负责人可立即撤销，其他销售受7天保护。 |
