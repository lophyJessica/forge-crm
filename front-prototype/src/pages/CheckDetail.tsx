...
export default function CheckDetail() {
  ...
  if (loading) {
    return <div className="forge-state-panel">正在解析复核数据...</div>;
  }

  if (!detail) {
    return <div className="forge-state-panel forge-state-panel--error">该复核单不存在</div>;
  }
  ...
  return (
    <div className="space-y-4 text-xs pb-8">
      <PageHeader
        onBack={() => navigate('/outbound/checks')}
        title={<><span className="font-mono">{detail.id}</span><span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getStatusClasses(detail.status)}`}>{detail.status === 'PENDING' ? '待复核' : '已完成'}</span></>}
        description={<span className="font-mono">关联出库波次：{detail.waveOrderId}</span>}
        actions={detail.status === 'PENDING' ? (
          <Button size="sm" onClick={() => navigate(`/outbound/${detail.waveOrderId}/checking`)} className="bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1.5 font-bold cursor-pointer">
