...
import { AlertTriangle, ArrowLeft, Calendar, CheckCircle2, ClipboardX, XCircle } from 'lucide-react';
...
export default function DamageDetail() {
  ...
  const handleConfirm = async () => {
    if (!order || !window.confirm(`确定要确认报损单 ${order.id} 吗？确认后将立即扣减现存并生成 FL 流水。`)) return;
    try {
