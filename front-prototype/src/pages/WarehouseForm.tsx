  ...
  return (
    <div className="space-y-4 max-w-3xl text-xs">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-900">{isEdit ? `编辑仓库 ${code}` : '新增仓库档案'}</h1>
          <p className="text-xs text-slate-500 mt-1">仓库编码新增时自填，保存后作为主数据唯一标识</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="bg-white border border-slate-200 rounded-lg p-5 shadow-sm space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-1 md:col-span-2">
            <label className="font-semibold text-slate-500">地址</label>
            <Input
              value={form.address}
