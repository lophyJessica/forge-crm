...
export default function WarehouseList() {
  ...
  const handleStatusChange = async (row: Warehouse) => {
    ...
    if (!window.confirm(`确认${actionName}仓库 ${row.name} 吗？`)) return;
      ...
      <form onSubmit={e => { e.preventDefault(); loadData(); }} className="bg-white rounded-lg border border-slate-200 p-4 shadow-sm space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
          ...
          <div className="space-y-1">
            ...
            <select value={type} onChange={e => setType(e.target.value as WarehouseType | '')} className="w-full h-9 rounded-md border border-input bg-background px-3 py-1 focus-visible:outline-none focus-visible:ring-2">
          ...
          <div className="space-y-1">
            ...
            <select value={status} onChange={e => setStatus(e.target.value as BaseDataStatus | 'ALL')} className="w-full h-9 rounded-md border border-input bg-background px-3 py-1 focus-visible:outline-none focus-visible:ring-2">
              ...
              {rows.length === 0 ? (
                ...
                <tr key={row.code} className="hover:bg-slate-50/50">
                  ...
                  <td className="p-3 text-center space-x-1 whitespace-nowrap">
                    <Button variant="ghost" size="sm" className="h-7 text-blue-600 hover:bg-blue-50" onClick={() => navigate(`/base/warehouses/${row.code}/edit`)}>
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className={`h-7 ${row.status === 'ENABLED' ? 'text-amber-600 hover:bg-amber-50 font-bold' : 'text-emerald-600 hover:bg-emerald-50 font-bold'}`}
