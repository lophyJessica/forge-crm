  ...
  return (
    <div className="space-y-4 text-xs">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold text-slate-900">出库复核单</h1>
          <p className="text-xs text-slate-500 mt-1">对拣货完毕的实物商品进行二次扫描复核，验证条码、商品一致性及出库数量</p>
        </div>
      </div>
      ...
      <div className="border-b border-slate-200 flex justify-between items-end">
        <div className="flex gap-1 text-sm font-medium">
          {(['ALL', 'PENDING', 'COMPLETED'] as const).map(tab => {
            const labelMap = { ALL: '全部', PENDING: '待复核', COMPLETED: '已完成' };
                    ...
                    <td className="p-3 text-center space-x-1">
                      ...
                      {row.status === 'PENDING' && (
                        <Button
                          ...
                          onClick={() => navigate(`/outbound/waves/${row.waveOrderId}/check`)}
                        >
