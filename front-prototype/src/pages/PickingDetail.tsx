...
import { ArrowUpCircle, Calendar, Layers, Clipboard, ShieldCheck, CheckCircle2 } from 'lucide-react';
...
export default function PickingDetail() {
  ...
  if (loading) {
    return <div className="forge-state-panel">正在解析拣货明细...</div>;
  }

  if (!detail) {
    return <div className="forge-state-panel forge-state-panel--error">该拣货单不存在</div>;
  }
  ...
  return (
    <div className="space-y-4 text-xs pb-8">
      <PageHeader
        onBack={() => navigate('/outbound/pickings')}
        title={<><span className="font-mono">{detail.id}</span><span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${getStatusClasses(detail.status)}`}>{detail.status === 'PICKING' ? '拣货中' : '已完成'}</span></>}
        description={<span className="font-mono">关联出库波次：{detail.waveOrderId}</span>}
        actions={detail.status === 'PICKING' ? (
          <Button size="sm" onClick={() => navigate(`/outbound/${detail.waveOrderId}/picking`)} className="bg-orange-600 hover:bg-orange-700 text-white flex items-center gap-1.5 font-bold cursor-pointer">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between gap-3">
                ...
                <span className="font-mono text-primary hover:underline">
                  <Link to={`/outbound/${detail.waveOrderId}`}>{detail.waveOrderId}</Link>
                </span>
              </div>
              <div className="flex justify-between gap-3">
