...
/*
  ...
  const loadData = async () => {
    ...
    try {
      const res = await inboundApi.getInbounds({
        ...
        productCode: productKeyword || undefined,
      });
      ...
      // 计算 Tab 数量计数
      const allStatuses: (InboundStatus | 'ALL')[] = ['ALL', 'DRAFT', 'RECEIVING', 'QC_PENDING', 'PUTAWAY_PENDING', 'EXCEPTION', 'COMPLETED', 'VOIDED'];
  ...
  useEffect(() => {
    loadData();
  }, [activeTab, statusFilter, supplierCode, warehouseCode, receiveDateStart, receiveDateEnd, updatedDateStart, updatedDateEnd, isExpanded, productKeyword]);
  ...
  const handleQCResult = async (isPassed: boolean) => {
    ...
    try {
      ...
      setQcOrder(null);
      loadData();
      alert(isPassed ? '质检合格判定成功，现可开始上架流程' : '质检判定不合格，已自动将单据置为异常等待退货');
    } catch (err: any) {
  ...
  const handlePrintRow = (row: InboundOrder, e: React.MouseEvent) => {
    e.stopPropagation();
    alert(`收货单 [${row.id}] 标签打印成功！共计打印商品条码和托盘标签 ${row.items.reduce((sum, i) => sum + Number(i.receivedQuantity || 0), 0)} 张。`);
  };

  const handleBatchPrint = () => {
    if (selectedIds.length === 0) {
      alert('请先勾选需要打印标签的收货单');
      return;
    }
    const selectedOrders = inbounds.filter(o => selectedIds.includes(o.id));
    const invalidOrders = selectedOrders.filter(o => 
      o.status === 'DRAFT' || o.status === 'RECEIVING' || o.status === 'VOIDED'
    );
    if (invalidOrders.length > 0) {
      alert('只有“待质检”、“待上架”、“异常”、“已完成”状态的单据可以打印标签，请重新选择！');
      return;
    }

    const printCounts = selectedOrders.reduce((sum, o) => sum + o.items.reduce((s, i) => s + Number(i.receivedQuantity || 0), 0), 0);
    alert(`批量发送标签打印成功！\n打印单据：${selectedOrders.map(o => o.id).join(', ')}\n累计打印商品条码与面单 ${printCounts} 张。`);
  };
  ...
  const getStatusBadge = (status: InboundStatus) => {
    const config: Record<InboundStatus, { label: string; classes: string }> = {
      DRAFT: { label: '待收货', classes: 'bg-zinc-100 text-zinc-800 border-zinc-200' },
      RECEIVING: { label: '收货中', classes: 'bg-blue-50 text-blue-700 border-blue-200' },
      QC_PENDING: { label: '待质检', classes: 'bg-amber-50 text-amber-700 border-amber-200 animate-pulse' },
      PUTAWAY_PENDING: { label: '待上架', classes: 'bg-orange-50 text-orange-700 border-orange-200' },
      EXCEPTION: { label: '异常', classes: 'bg-rose-50 text-rose-700 border-rose-200 font-bold' },
      COMPLETED: { label: '已完成', classes: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
      VOIDED: { label: '已作废', classes: 'bg-slate-100 text-slate-400 border-slate-200' }
    };
    ...
    <div className="space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={handleBatchPrint} className="flex items-center gap-1.5 cursor-pointer text-orange-600 border-orange-200 hover:bg-orange-50 font-bold">
            <span>批量打印标签</span>
          </Button>
      ...
      <form onSubmit={handleSearch} className="bg-white rounded-lg border border-slate-200 p-4 shadow-sm space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-8 gap-3">
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-500">商品编码/SKU</label>
            <Input 
              placeholder="输入SKU..." 
          ...
          <div className="space-y-1">
            ...
            >
              ...
              <option value="DRAFT">待收货</option>
              <option value="RECEIVING">收货中</option>
              <option value="QC_PENDING">待质检</option>
              <option value="PUTAWAY_PENDING">待上架</option>
                  ...
                  return (
                    <tr key={row.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-3 text-center space-x-1">
                        ...
                        {(row.status === 'DRAFT' || row.status === 'RECEIVING') && (
                          <Button
                            variant="ghost"
                            size="sm"
                        ...
                        {row.status === 'QC_PENDING' && (
                          <>
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-7 text-slate-600 hover:bg-slate-50"
                              onClick={(e) => handlePrintRow(row, e)}
                            >
                              <span>打印标签</span>
                            </Button>
                          </>
                        ...
                        {(row.status === 'PUTAWAY_PENDING' || row.status === 'EXCEPTION' || row.status === 'COMPLETED') && (
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 text-slate-600 hover:bg-slate-50"
          ...
          <div className="bg-white rounded-lg border border-slate-200 max-w-md w-full shadow-lg p-5">
            ...
            <div className="bg-yellow-50 text-yellow-700 p-3 rounded border border-yellow-200 text-xs mt-3 leading-relaxed">
              ...
              <ul className="list-disc list-inside mt-1 space-y-1">
                <li>合格判定：商品解封转入正常待上架库，状态变更为“待上架”，可由上架单进行后续货位上架。</li>
                <li>不合格判定：判定全部退货并销账，本收货单状态转为“异常”等待退货，冲减对应的冻结库存。</li>
              </ul>
            </div>
            <div className="flex justify-end gap-2 mt-5">
              ...
              <Button size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer font-bold" onClick={() => handleQCResult(true)}>
                判定合格(确认放行)
              </Button>
            </div>
          </div>
