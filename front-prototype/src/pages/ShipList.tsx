  ...
  return (
    <div className="space-y-4 text-xs">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold text-slate-900">出库交运单</h1>
          <p className="text-xs text-slate-500 mt-1">处理打包完成的交运包裹移交快递/物流承运商，交运确认只完结订单/回传，不扣减库存</p>
        </div>
      </div>
      ...
      <div className="border-b border-slate-200 flex justify-between items-end">
        <div className="flex gap-1 text-sm font-medium">
          {(['ALL', 'PENDING', 'COMPLETED'] as const).map(tab => {
            const labelMap = { ALL: '全部', PENDING: '待交运', COMPLETED: '已完成' };
                    ...
                    <td className="p-3 text-center space-x-1">
                      ...
                      {row.status === 'PENDING' && (
                        <Button
                          ...
                          onClick={() => navigate(`/outbound/waves/${row.waveOrderId}/ship`)}
                        >
